...
public class MapColor {
    ...
    public static MapColor get(int id) {
        checkPositionIndex(id, COLORS.length, "material id");
    ...
    public boolean equals(Object o) {
        ...
        MapColor mapColor = (MapColor) o;
        return color == mapColor.color && id == mapColor.id;
    }
    ...
    public enum Brightness {
        ...
        public static Brightness validateAndGet(int id) {
            checkPositionIndex(id, VALUES.length, "brightness id");
            return get(id);
        }
    ...
    private static void checkPositionIndex(int index, int size, String desc) {
        if (index < 0 || index > size)
            throw new IndexOutOfBoundsException("index %s < 0 or index %s > size %s. %s".formatted(index, index, size, desc));
    }
