...
public class ChunkPos {
    ...
    public boolean checkDistance() {
        if (mc.settings.getChunkDistance() == 0) return false;
        ChunkPos playerPos = mc.player.getChunkPos();
        return getSquaredDistance(playerPos) <=  Math.pow(mc.settings.getChunkDistance(), 2);
    }
    ...
    public double getSquaredDistance(ChunkPos pos) {
        return getSquaredDistance(pos.x, pos.z);
    }

    public double getSquaredDistance(int x, int z) {
        double a = (double) this.x - x;
        double b = (double) this.z - z;
        return a * a + b * b;
    }
